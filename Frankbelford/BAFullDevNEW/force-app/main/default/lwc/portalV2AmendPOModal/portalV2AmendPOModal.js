...
export default class PortalV2AmendPOModal extends LightningElement {
  ...
  async handleApplyClicked() {
    ...
    this.shiftRecords.forEach((record) => {
      if (record.timesheet) {
        console.log('inside if record.timesheet::'+record.timesheet);
        updatedRecords.push(
          UpdateInvoicePO({
            timeId: record.id,
        ...
        console.log('inside updated records' + JSON.stringify(updatedRecords));   
      }
    });
    ...
    console.log(result);
    this.loading = false;
    console.log('result po update ::'+JSON.stringify(result));
