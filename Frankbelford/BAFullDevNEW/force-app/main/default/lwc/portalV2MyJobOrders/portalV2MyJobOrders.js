...
import TS2_MAILINGSTREET_FIELD from '@salesforce/schema/Contact.MailingStateText__c';
import TS2_MAILINGCOUNTRY_FIELD from '@salesforce/schema/Contact.MailingCountryText__c';
...
export default class PortalV2MyJobOrders extends LightningElement {
  ...
  @track activeSection = 'Week 1';

  handleAccordionChange(event) {
    console.log('accordion ebvent:'+JSON.stringify(event));
 ...
 // daysOfWeek = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
        ...
        if (this.hasRecords === true) {
          this.allJobOrderRecords = results.map(jorec =>{
            return {
              ...
              joDeclinedatetime: jorec.Job_Order_Decline_Details__r !=undefined ? jorec.Job_Order_Decline_Details__r[0].Declined_Date_Time__c :''
            };
          });
          this.flattenShifts();
          this.filterRows();
  ...
  flattenShifts() {
    try {
      ...
      jobOrderRecordsCopy.forEach((jobOrder) => {
        ...
        let enddate = this.formatDate(jobOrder.sirenum__EndDate__c);
  ...
  get joDeclinedatetime() {
    return this.joborderdetaildata.Job_Order_Decline_Details__r[0].Declined_Date_Time__c;
}
    ...
    getworkschedules({ workschid: this.joborderdetaildata.sirenum__WorkSchedule__c })
      .then(results => {
        ...
        if (this.hasWorkScheduleRecords === true) {
          this.allWorkScheduleRecords =results;
          const length = results[0].sirenum__Length__c !=undefined ? results[0].sirenum__Length__c : 7;
          const alignment = results[0].sirenum__Alignment__c !=undefined ? results[0].sirenum__Alignment__c :"Weekly - Monday";
          this.flattenWorkSchedule = this.fillMissingDays(results[0].sirenum__Work_Schedule_Days__r,length,alignment);
          console.log('**********flattenWorkSchedule: ', JSON.stringify(this.flattenWorkSchedule));
  ...
  fillMissingDays(data, length, alignment) {
    console.log('details send=' + JSON.stringify(data) + 'length=' + length + 'alignment=' + alignment);
    const dataMap = new Map(data.map(item => [item.sirenum__Offset__c, item]));
    const weekCount = length / 7;
    console.log('weekcount',weekCount);
  
    // Extract starting day from alignment string
    const startDay = alignment.split(' - ')[1].trim();
    const baseDays = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
  
    // Rotate daysOfWeek based on alignment
    const startIndex = baseDays.indexOf(startDay);
    const daysOfWeek = [...baseDays.slice(startIndex), ...baseDays.slice(0, startIndex)];
  
    const weekGroups = [];
  
    for (let w = 0; w < weekCount; w++) {
      console.log('inside week:'+w);
      const weekLines = [];
  
      for (let d = 0; d < 7; d++) {
        console.log('inside lines:'+d);
        const index = w * 7 + d;
        const day = daysOfWeek[d];
  
        const baseItem = dataMap.has(index) ? {
          ...dataMap.get(index),
          dayOfWeek: day,
          formattedStartTime: this.formatTime(dataMap.get(index).sirenum__Start__c),
          formattedEndTime: this.formatTime(dataMap.get(index).sirenum__End__c)
        } : {
          Id: null,
