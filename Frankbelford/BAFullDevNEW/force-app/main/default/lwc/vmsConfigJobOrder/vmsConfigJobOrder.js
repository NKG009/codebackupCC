...
import getJobOrderss from '@salesforce/apex/VMSConfigController.getJobOrderss';
...
import getsingleJobOrderforSite from '@salesforce/apex/VMSConfigController.getsingleJobOrderforSite';
import updateVendStatus from '@salesforce/apex/VMSConfigController.updateVendStatus';
import genrateLink from '@salesforce/apex/VMSConfigController.genrateLink';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';
import updateJobOrdersApex from '@salesforce/apex/VMSConfigController.updateJobOrders';

export default class VmsConfigJobOrder extends LightningElement {
    ...
    /*
        ...
        @wire(getSitesForAccount, { accountId: 'this.jobAccountId', searchTerm: '$searchTerm' })
    ...
    @track orderIdss;
    @wire(getJobOrderss, { jobOrderId: '$recordId' })
    wiredJobOrders({ error, data }) {
        if (data) {
            console.log('====================data new single' + JSON.stringify(data));
            const jobOrder = data[0]; // Access the first element of the array
            this.jobAccountId = jobOrder.sirenum__Account__c;
            this.selectedSiteId = jobOrder.sirenum__Site__c;
            this.orderIdss = jobOrder.Id;
            this.getSiteData();
        } else if (error) {
    ...
    getSuppliersData() {
        ...
        if (!this.jobAccountId || !this.selectedSiteId) {
            console.error('Missing required parameters: Account ID or Site ID.');
            return;
        }

        getSuppliersForAccount({ accountId: this.jobAccountId, siteId: this.selectedSiteId })
            .then((data) => {
                ...
                this.allSuppliers = data.map(supplier => {
                    ...
                    return {
                        ...
                        tierValue: tierValue, // Save tierValue to use in sorting
                        tierOption: [
    ...
    getJobOrderData() {
        ...
        getsingleJobOrderforSite({ accountId: this.jobAccountId, siteId: this.selectedSiteId, jobOrderId: this.orderIdss })
            .then((data) => {
                console.log('==================Data received from getsingleJobOrderforSite:', JSON.stringify(data));
                this.allJobOrders = [];
                if (!data || data.length === 0) {
    ...
    handelVendClick(event) {
        ...
        genraetLink({ siteId: siteId, supplierId: supplierId })
            .then((message) => {
                this.showToast('Success', message, 'success');
                this.getSuppliersData();
            })
    ...
    genrateLink(event) {
        ...
        this.isLoading = true;
        console.log('==========================this.selectedSiteId ' + this.selectedSiteId);
        console.log('==========================this.selectedSupplierId ' + this.selectedSupplierId);
        genrateLink({ siteId: siteId, supplierId: supplierId })
            .then((message) => {
                this.showToast('Success', message, 'success');
                this.getSuppliersData();
                this.isLoading = false;
