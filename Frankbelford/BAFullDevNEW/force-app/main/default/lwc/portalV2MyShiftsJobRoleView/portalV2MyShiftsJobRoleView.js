...
export default class PortalV2JobRoleView extends LightningElement {
  pageRef;
  ...
  async getShifts() {
    ...
    try {
      ...
      console.log('===================================== old : '+JSON.stringify(results));
  ...
  async getCurrentShifts() {
    ...
    console.log('==============================================================this.shifts old==='+JSON.stringify(this.shifts));
  }
  ...
  generateTopLevelShift(shift, ratings) {
    ...
    const shiftObject = {
      ...
      get numFilled() {
        ...
        return this.shifts
          .map((record) => {
            return record.numFilled ? record.numFilled : 0;
          })
          .reduce((a, b) => a + b);
      },
      get fulfilment() {
      ...
      get totalShifts() {
        return this.shifts
          .map((record) => record.totalRequested)
          .reduce((a, b) => a + b);
      },
      hidden: true,
      shifts: []
  ...
  generateChildShift(shift, parent, ratings) {
    ...
    const shiftObject = {
      ...
      numFilled: shift.sirenum__Broadcasts__c
        ? shift.sirenum__AssignedShifts__c
      ...
      get fulfilmentClass() {
        const remaining = shift.sirenum__Broadcasts__c
          ? shift.Remaining_Shifts__c
          : shift.sirenum__Contact__r
          ? 0
          : 1;
        switch (remaining) {
      ...
      get fulfilment() {
        ...
        if (shift.sirenum__Broadcasts__c) {
          fulfilmentString = `${shift.sirenum__AssignedShifts__c ? shift.sirenum__AssignedShifts__c : 0} of ${shift.sirenum__Broadcasts__c}`;
        } else {
    ...
    if (shift.sirenum__Shifts__r) {
      shift.sirenum__Shifts__r.forEach((demandChild) => {
        this.generateChildShift(demandChild, shiftObject, ratings);
      });
    }
  ...
  handleApplyFilter(event) {
    console.log('================================= handleColumPickerFilter : ');
