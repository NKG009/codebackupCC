...
export default class PortalV2ContactUs extends LightningElement {
 ...
 /* async getSubjectOptions() {
  ...
  if (this.vmscontact) {
    // ✅ Hardcoded options for VMS contacts
    console.log('VMS contact is true — using hardcoded options');
    this.subjectOptions = [
      { label: 'Job Order Query', value: 'Job Order Query' },
      { label: 'Hour Submission Query', value: 'Hour Submission Query' },
      { label: 'Timesheet Query', value: 'Timesheet Query' }
    ];
  } else {
    // 🔁 For non-VMS contacts, fetch from Apex
    console.log('VMS contact is false — loading options from Apex');
    const options = await getselectOptionsClient({
      objObject: "{sobjectType : 'Case'}",
      fld: "Subject_Client_Query__c"
    });
    ...
    options.forEach((option) => {
      console.log('Processing Apex option:', option);

      // Skip VMS-only options
      if (
        option !== 'Job Order Query' &&
        option !== 'Hour Submission Query' &&
        option !== 'Timesheet Query'
      ) {
        this.subjectOptions = [
          ...this.subjectOptions,
          {
  ...
  console.log('Final subjectOptions:', JSON.stringify(this.subjectOptions));
}*/
