trigger DVLACheckTask on Certification__c (after insert, after Update,before delete,before insert,before update) {
    if(trigger.isbefore && trigger.isdelete)
    {
        Id profileId=userinfo.getProfileId();
        String profileName=[Select Id,Name from Profile where Id=:profileId].Name;
        for(Certification__c checkdelete:trigger.old)
        {
    ...
    //=====JPC-2553 Start=========
    List<Certification__c> Listforlaps = new List<Certification__c>();
    set<id> contactid = new set<id>();
    set<id> certiList = new set<id>();
    Map<string,Certification__c> maptocompair = new map<string,Certification__c>();
    ...
    if(Trigger.isbefore && trigger.isinsert)
    {
        ...
        {
            string rectypename= Schema.getGlobalDescribe().get('Certification__c').getDescribe().getRecordTypeInfosById().get(c.RecordTypeId).getName();
            if(c.Verfied__c && rectypename != 'Right to Work' ){
                contactid.add(c.Contact__c);
                certiList.add(c.id);
                string key = string.valueof(c.Contact__c)+ string.valueof(c.RecordTypeId);
    ...
    if(Trigger.isbefore && trigger.isupdate)
    {
        for(Certification__c c: trigger.new) 
        {
            Certification__c oldrec = Trigger.OldMap.get(c.id);
            string rectypename= Schema.getGlobalDescribe().get('Certification__c').getDescribe().getRecordTypeInfosById().get(c.RecordTypeId).getName();
    ...
    Listforlaps = [Select id,IP_Status__c,Verfied__c,Contact__c,RecordTypeId from Certification__c where IP_Status__c = 'Current' and Contact__c in:contactid and id not in:certiList];
    list<Certification__c> listtoupdate = new list<Certification__c>();
    for(Certification__c laps :Listforlaps)
    {
        string keytocomp = string.valueof(laps.Contact__c)+ string.valueof(laps.RecordTypeId);
    ...
    if(trigger.isAfter && trigger.isUpdate)
    {
        ...
        for(Certification__c cert : trigger.new){
            ...
            {
                conmap.put(cert.Contact__c,cert.recordtypeId);
                conId.add(cert.Contact__c);
        ...
        if(conId.size()>0)
        {
            ...
            list<contact> lapseconlist = [Select id,PORTW_Completed__c,(select id,IP_IdentificationValidFromDate__c,Citizenship_group__c,VISA_Expiration_Date__c,
                                                                        ...
                                                                        from Certifications__r where IP_Status__c = 'Current' and recordtypeid in:conmap.values() and
            ...
            for(Contact c: lapseconlist){
                if(recordtypeName.contains('Right to Work'))
                {
                    ...
                    if(c.Certifications__r.size() >0)
                    {
                        
                        c.PORTW_Completed__c = TRUE;
                        c.IP_CitizenshipGroup__c = c.Certifications__r[0].Citizenship_group__c;
                        if(c.Certifications__r[0].IP_PORTWStartDate__c == NULL)
                        {
                            c.IP_PORTWStartDate__c=c.Certifications__r[0].IP_IdentificationValidFromDate__c;
                        }
                        else
                        {
                            c.IP_PORTWStartDate__c=c.Certifications__r[0].IP_PORTWStartDate__c;
                        }
                        if(c.Certifications__r[0].VISA_Type__c == 'Resident Card (with time limit)')
                        {
                            c.Passport_Expiration_Date__c=c.Certifications__r[0].VISA_Expiration_Date__c;
                        }
                        else
                        {
                            c.Passport_Expiration_Date__c=c.Certifications__r[0].Identification_Expiry_Date__c;
                        }
                        
                        if(c.Certifications__r[0].Identification_Type__c == 'ECS check')
                        {
                            c.VISA_Expiration_Date__c=NULL;
                        }
                        else
                        {
                            c.VISA_Expiration_Date__c=c.Certifications__r[0].VISA_Expiration_Date__c;
                        }
...
tsk.whoid = trigger.new[i].Contact__c;
