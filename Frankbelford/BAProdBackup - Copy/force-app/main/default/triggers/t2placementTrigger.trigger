trigger t2placementTrigger on ts2__Placement__c (before insert,before update,after insert,after update, before delete,after delete,after undelete) {
    ...
    if(Trigger.isAfter){ 
        ...
        for(ts2__Placement__c t2d:Trigger.new){
            if(t2d.IP_PassToTempest__c==true){
                idsTrue.add(t2d.ts2__Hiring_Manager__c); 
                         
            }else {
                idsfalse.add(t2d.ts2__Hiring_Manager__c);                    
            }
            if(Trigger.isInsert)
            {
                 if(t2d.IP_PassToTempest__c==true){
                 ...
                 {
                     conIds.add(t2d.ts2__Employee__c);
                 }
                
            }
            if(Trigger.isUpdate)
            {
                ts2__Placement__c oldplace = Trigger.oldMap.get(t2d.Id);
        ...
        if(Trigger.isDelete)
        {
            for(ts2__Placement__c p: Trigger.old)
            {
                conIds.add(p.ts2__Employee__c);
            }
        }
    }
    ...
    if(Trigger.isBefore)
    {
        if(Trigger.isInsert)
            {
              ...
              //  if(t2d.IP_DateAvailableforRecontact__c != null)
              //  {
                if(t2d.ts2extams__Substatus__c == 'Hired')
                {
                    t2d.IP_StatuschangedtoHired__c = System.today();
                    if(t2d.ts2__Start_Date__c != null)
                    {
                        t2d.IP_DateAvailableforRecontact__c = t2d.ts2__Start_Date__c+365;
                    }
                    else
                    {
                ...
                if(t2d.ts2extams__Substatus__c == 'Placed: Cancelled by Candidate' || t2d.ts2extams__Substatus__c == 'No Show')
                {
        ...
        if(Trigger.isUpdate)
        {
            for(ts2__Placement__c t2d:Trigger.new){
                ts2__Placement__c oldplace = Trigger.oldMap.get(t2d.Id);
                if(oldplace.ts2extams__Substatus__c != t2d.ts2extams__Substatus__c)
                {
                    if(t2d.ts2extams__Substatus__c == 'Hired')
                    {
                           t2d.IP_StatuschangedtoHired__c = System.today();
                ...
                if(oldplace.ts2__Start_Date__c != t2d.ts2__Start_Date__c && t2d.ts2__Start_Date__c != null)
                {
                    t2d.IP_DateAvailableforRecontact__c = t2d.ts2__Start_Date__c+365;
                }
            }
    ...
    if(Trigger.isBefore && Trigger.IsDelete)
    {
     ...
     if(!conIDs.isEmpty())
     {
     list<AggregateResult > Outputvalues = [select ts2__Employee__c Con,Max(IP_DateAvailableforRecontact__c) Maxdata from ts2__Placement__c where ts2__Employee__c IN:conIds group by ts2__Employee__c];
    ...
    if(Trigger.isUpdate && Trigger.isBefore){
        ...
        for(ts2__Placement__c placemnt : Trigger.New){
            ts2__Placement__c oldpl = Trigger.oldMap.get(placemnt.Id);
            ...
            {  
                if(placemnt.ts2extams__Substatus__c == 'Rebate Period' && placemnt.Name !=NULL && placemnt.ts2__Start_Date__c  != NULL && (placemnt.ts2__Fee_Pct__c !=NULL || placemnt.IP_Fixed_Fee__c != 0 )&& ( placemnt.ts2__Salary__c !=0 && placemnt.ts2__Salary__c != NULL) 
                   && placemnt.ts2__Filled_By__c != NULL && placemnt.IP_PermID__c !=NULL && placemnt.ts2__Filled_Pct__c !=0  
