  ...
  async fetchJobOrderRecords() {
    ...
    getAllJobOrders()
      .then((results) => {
        ...
        const ongoingOrders = results.filter(order => order.sirenum__Status__c === 'Ongoing');
        console.log(
          "result lenth: " + results.length);
          console.log(
            "open job length: " +  ongoingOrders.length);
            
        this.openJobOrders = ongoingOrders.length;
        this.loading=false;
