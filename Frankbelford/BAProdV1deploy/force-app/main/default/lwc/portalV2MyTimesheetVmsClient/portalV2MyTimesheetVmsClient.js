...
export default class PortalV2MyTimesheetVmsClient extends LightningElement {
  pageRef;
  ...
  get feedbackTargets() {
    ...
    if (this.jobView) {
      const jobRoleView = this.template.querySelector(
        "c-portal-v2-my-timesheet-vms-client-child"
      );
  ...
  get mainSelectedRows() {
    console.log('==================================== this.jobView : '+this.jobView);
    if (this.jobView) {
      const jobRoleView = this.template.querySelector(
        "c-portal-v2-my-timesheet-vms-client-child"
      );
