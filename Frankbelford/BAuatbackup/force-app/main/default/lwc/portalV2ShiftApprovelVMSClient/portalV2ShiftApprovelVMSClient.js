...
import getAllShiftRecords from "@salesforce/apex/PortalV2HoursSubmission.getAllShiftRecordsVMSClient";
...
export default class PortalV2ShiftApprovelVMSClient extends NavigationMixin(
  LightningElement
) {
  //#region Variables
  timeZone = TIME_ZONE;
  ...
  async fetchTimesheetRecords() {
    ...
    getAllShiftRecords({
      ...
      .then((results) => {
        console.log('==========================results flow check'+JSON.stringify(results));
  ...
  @track hasZeroBreakFlag = false;

  openEndorseApprovalModalBox() {
    this.hasZeroBreakFlag = false;
    ...
    if (this.selectedShiftRecords.length > 0) {
      this.selectedShiftRecords.forEach((value) => {
        if (value) {
          ...
          let isZeroBreak = value.Break_Length__c === '00:00';
          console.log('============== value.Break_Length__c', value.Break_Length__c);
          console.log('============== isZeroBreak', isZeroBreak);
          if (isZeroBreak) {
            this.hasZeroBreakFlag = true; 
          } 
          console.log('============== this.hasZeroBreakFlag', this.hasZeroBreakFlag);
